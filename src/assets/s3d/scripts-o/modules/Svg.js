...
class Svg {
	...
	getData(fn) {
		...
		// })
		$.ajax('/wp-content/themes/idealist/assets/s3d/svg.json').done(msg => {
	...
	createSvg(data, name) {
		const svgContainer = createMarkup('div', `#js-s3d__wrapper__${this.idCopmlex}`, { class: `js-s3d__svg-container js-s3d__svg-container${name === 'complex' ? '__complex' : name}` })
